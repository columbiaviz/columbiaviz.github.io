...
5.  **Midterm Exam (8%):** individual closed book exam, for 75 min. taken online via Canvas this semester. Comprises multiple-choice and written essay questions.
...
- **Group Work**: All assignments will be in groups of 3~4 unless otherwise noted. In general, I do not anticipate that the grades for individual group members will be different. However, we reserve the right to assign different grades to each group member based on [peer assessments]({{ layout.peerassessform }}). Your assignments are all due [online]({{ layout.submissionform }}) at 11:59pm Eastern Time the Monday *before* class unless otherwise specified.
